function [mxyz,idx]=surf_project(xyz,filenames,matched,file_patch,file_offset,file_scale)
% SURF_PROJECT
% mxyz = surf_project(xyz,filenames,matched)
%  xyz:         original coordinates
%  filenames:   surface(s) (target coordinates)
%  matched:     0: match xyz coordinates to surface coordinates directly
%               1: match xyz coordinates to fsaverage pial.surf coordinates first 
%                  and then to corresponding surface coordinates
%               filenames2: match xyz coordinates to these surfaces first
%                  and then to corresponding surface coordinates
%  mxyz:        matched coordinates
% 
...
for n1=1:numel(filenames)
    if nargin>=4&&numel(file_patch)>=n1
        ...
        if nargin>=6
            mp=mean(xyz0{n1},1);
            xyz0{n1}=bsxfun(@plus,mp+file_offset{n1},bsxfun(@minus,xyz0{n1},mp)*file_scale{n1});
        end
    else
        xyz0{n1}=read_surf(filenames{n1});
    end
end
if ischar(matched), matched=cellstr(matched); end
if iscell(matched)
    if numel(filenames)~=numel(matched), error('incorrect number of matched files'); end
    filenamesmatched=matched;
    matched=true;
else filenamesmatched={};
end
if matched
    if isempty(filenamesmatched),
    ...
    for n1=1:numel(filenames1)
        xyz0{n1}=read_surf(filenames1{n1});
    end
else
end
...
idx=d;
for n1=1:numel(xyz0),
    [d(:,n1),idx(:,n1)]=min(permute(sum(abs(bsxfun(@minus,xyz0{n1},permute(xyz,[3,2,1]))).^2,2),[3,2,1]),[],3);
end
[mind,minidx]=min(d,[],2);
mind=sqrt(mind);
